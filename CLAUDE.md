...
- ✅ Готово: приём ПУД файлом → извлечение структуры → ведомость → ввод (кнопки) → расчёт → Excel.
- 🔜 Заглушки к реализации: голос (SpeechKit) и текст-поток (Qwen) — оба вливаются в тот же
  конвейер записи оценок; интеграция с Яндекс.Таблицами (цель по ТЗ: 5 ведомостей).
